...
namespace TeknoramaUI.Areas.Administration.Controllers
{
    ...
    public class SupplierController : Controller
    {
        ...
        public async Task<IActionResult> List()
        {
            ...
            var response = await client.GetAsync("http://localhost:5288/api/Suppliers");
        ...
        {
            var client = CreateClient();
            await client.DeleteAsync($"http://localhost:5288/api/Suppliers/{id}");
        ...
        public async Task<IActionResult> Create(SupplierCreateRequestModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var response = await client.PostAsync("http://localhost:5288/api/Suppliers", requestContent);
                return RedirectToAction("List");
            }
            return View(model);
        }
        ...
        {
            var client = CreateClient();
            var response = await client.GetAsync($"http://localhost:5288/api/Suppliers/{id}");
            if (response.IsSuccessStatusCode)
            {
                var jsonString = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> Update(SupplierUpdateRequestModel model)
        {
            if (ModelState.IsValid)
            {
                var client = CreateClient();
                var requestContent = new StringContent(JsonSerializer.Serialize(model), Encoding.UTF8, "application/json");
                var response = await client.PutAsync("http://localhost:5288/api/Suppliers", requestContent);
                return RedirectToAction("List");
            }
            return View(model);
        }
